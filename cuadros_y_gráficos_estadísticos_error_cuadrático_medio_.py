...
Original file is located at
    https://colab.research.google.com/drive/1zjJgK0lEv6qpXHwsSksZLGaj_5cv4wAA
...
#Incisos a y b
# Datos de ejemplo (porcentajes de juegos ganados)
porcentajes = np.array([0.688, 0.688, 0.563, 0.250, 0.125, 0.688, 0.625, 0.438, 0.313, 0.688, 0.625, 0.500, 0.438, 0.125, 0.813, 0.688, 0.563, 0.375, 0.250, 0.688, 0.563, 0.313, 0.313, 0.313, 0.875, 0.750, 0.375, 0.375])


# Obtener las frecuencias absolutas y los límites de las clases
frecuencia_absoluta, bordes = np.histogram(porcentajes, bins=5)

# Total de elementos
total_datos = len(porcentajes)

# Calcular frecuencia relativa
frecuencia_relativa = frecuencia_absoluta / total_datos

# Mostrar las frecuencias absoluta y relativa
for i in range(len(frecuencia_absoluta)):
    print(f"Clase {i+1}: [{bordes[i]:.3f}, {bordes[i+1]:.3f}]")
    print(f"  Frecuencia absoluta: {frecuencia_absoluta[i]}")
    print(f"  Frecuencia relativa: {frecuencia_relativa[i]:.2%}\n")

import matplotlib.pyplot as plt

plt.hist(porcentajes, bins=5, edgecolor='black', alpha=0.7)
plt.xlabel("Porcentaje de juegos ganados")
plt.ylabel("Frecuencia absoluta")
plt.title("Distribución de porcentajes de juegos ganados")
plt.show()
...
# Graficar polígono de frecuencias absolutas
plt.figure(figsize=(8, 5))
plt.plot(puntos_medios, frecuencia_absoluta, marker='o', linestyle='-', color='b', label="Frecuencia Absoluta")

# Graficar polígono de frecuencias relativas
plt.plot(puntos_medios, frecuencia_relativa, marker='s', linestyle='--', color='r', label="Frecuencia Relativa")
...
frecuencia_acumulada_abs = np.cumsum(frecuencia_absoluta[::-1])[::-1]
frecuencia_acumulada_rel = frecuencia_acumulada_abs / len(porcentajes)  # Relativa

# Usamos los bordes superiores para el eje x (ojiva "mayor que")
bordes_superiores = bordes[1:]

# Graficar la ojiva de frecuencias absolutas
plt.figure(figsize=(8, 5))
plt.plot(bordes_superiores, frecuencia_acumulada_abs, marker='o', linestyle='-', color='b', label="Frecuencia Acumulada Absoluta")

# Graficar la ojiva de frecuencias relativas
plt.plot(bordes_superiores, frecuencia_acumulada_rel, marker='s', linestyle='--', color='r', label="Frecuencia Acumulada Relativa")

# Etiquetas y título
plt.xlabel("Porcentaje de juegos ganados")
plt.ylabel("Frecuencia Acumulada")
plt.title("Ojiva de Frecuencia Acumulada (Mayor que)")
plt.legend()
plt.grid(True)
